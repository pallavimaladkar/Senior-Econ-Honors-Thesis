...
global origin "~\Dropbox\CPS-shared\Emission"
...
import delimited using "Physical flow account for greenhouse gas emissions.csv", clear
drop dguid uom uom_id scalar_factor scalar_id vector coordinate status symbol terminated decimals
rename *ref_date year
label var year "Year"
rename geo province
...
drop if naics_emission=="Crop and animal production [BS11A00]" & year>2013
replace naics_emission="Crop and animal production [BS11A00]" if naics_emission=="Crop and animal production (except cannabis) [BS11B00]" & year>2013
drop if naics_emission=="Retail trade [BS4A000]" & year>2013
replace naics_emission="Retail trade [BS4A000]" if naics_emission=="Retail trade (except cannabis) [BS4AA00]" & year>2013

drop if substr(naics_emission, 1,9)=="Balancing"
drop if substr(naics_emission, 1,5)=="Total"
drop if substr(naics_emission, 1,8)=="Cannabis"
drop if substr(naics_emission, 1,10)=="Households"

drop if province == "Canada"
save emissions_ready.dta, replace

use emissions_ready.dta, clear
